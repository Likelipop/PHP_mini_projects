...
class Storage
{
    ...
    {
        if (self::$client === null) {
            self::$bucket = getenv('MINIO_BUCKET') ?: 'studyflow-assets';
    ...
    private static function ensureBucketExists(): void
    {
        try {
            ...
            // Silently skip or log in production
        }
    }
    ...
    {
        try {
            $externalEndpoint = getenv('MINIO_EXTERNAL_ENDPOINT') ?: 'http://localhost:9002';
            $accessKey = getenv('MINIO_ACCESS_KEY') ?: 'admin';
            $secretKey = getenv('MINIO_SECRET_KEY') ?: 'password123';
            $bucket = getenv('MINIO_BUCKET') ?: 'studyflow-assets';
